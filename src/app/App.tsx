...
export default function App() {
  ...
  // NUEVO: Estado para manejar el filtrado desde SubjectManager
  ...
  // Función para resetear filtros al navegar manualmente
        ...
        subjectConfig.sources.forEach(sourceConfig => {
          if (sourceConfig.questionCount > 0) {
            const subjectQuestions = allQuestions.filter(q =>
              q.subjectId === subjectConfig.subjectId && q.source === sourceConfig.source
            );
          ...
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center gap-3 cursor-pointer" onClick={() => navigateTo('dashboard')}>
              ...
              <div className="hidden sm:block">
                <h1 className="text-xl font-bold tracking-tight text-slate-800 dark:text-white">TestMaster</h1>
                <p className="text-[10px] uppercase tracking-widest opacity-50">Sistema de Evaluación</p>
              </div>
            </div>
            ...
            {navigationItems.map(item => (
              <button
                ...
                className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg text-left ${
                  currentView === item.id ? 'bg-blue-50 dark:bg-blue-900/20 text-blue-600' : 'text-slate-600 dark:text-slate-400'
                }`}
              >
      ...
      <main className="container mx-auto px-4 py-8">
        {!dataLoaded ? (
          <div className="flex flex-col items-center justify-center py-20">
            ...
            <p className="text-slate-500 animate-pulse">Sincronizando biblioteca de preguntas...</p>
          </div>
        ) : (
          <div className="max-w-6xl mx-auto">
            {currentView === 'dashboard' && <Dashboard onNavigate={navigateTo} />}
            
            {/* SUBJECTS: Ahora pasa las funciones para navegar y filtrar */}
            ...
            {/* QUESTIONS: Recibe el filtro opcional */}
            {currentView === 'questions' && (
              <QuestionEditor initialSubjectFilter={filterSubjectId} />
            )}

            {currentView === 'test-config' && <TestModeSelector onStartTest={handleStartTest} />}
            
            {currentView === 'test-active' && (
              <ActiveTest questions={testQuestions} onFinish={handleFinishTest} />
            )}
...
function Dashboard({ onNavigate }: { onNavigate: (view: ViewMode) => void }) {
  ...
  useEffect(() => {
    const loadStats = async () => {
      const { getSubjects, getQuestions, getTestResults } = await import('./utils/storage');
      setStats({
  ...
  const menuCards = [
    { id: 'subjects', title: 'Asignaturas', desc: 'Explora materias y temas', icon: BookOpen, color: 'bg-blue-500', count: stats.subjects },
    { id: 'questions', title: 'Banco de Preguntas', desc: 'Consulta el temario completo', icon: Edit3, color: 'bg-emerald-500', count: stats.questions },
    { id: 'test-config', title: 'Nuevo Examen', desc: 'Pon a prueba tus conocimientos', icon: Play, color: 'bg-orange-500' },
    { id: 'history', title: 'Tu Progreso', desc: 'Analiza tus resultados previos', icon: History, color: 'bg-purple-500', count: stats.tests },
  ];

  return (
    <div className="space-y-10 animate-in fade-in duration-700">
      <div className="text-center space-y-2">
        <h2 className="text-4xl font-extrabold tracking-tight text-slate-800 dark:text-white">Panel de Control</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {menuCards.map((card) => (
          ...
          >
            <div className={`${card.color} w-12 h-12 rounded-xl flex items-center justify-center text-white mb-4 group-hover:scale-110 transition-transform`}>
              <card.icon className="w-6 h-6" />
            </div>
            <h3 className="text-lg font-bold mb-1 text-slate-800 dark:text-white">{card.title}</h3>
