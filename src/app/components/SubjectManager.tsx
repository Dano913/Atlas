...
import { BookOpen, FileJson, GraduationCap, ChevronRight } from 'lucide-react';
import { Subject } from '../types';
import { getSubjects, getQuestionsBySubject } from '../utils/storage';

interface SubjectManagerProps {
  // Añadimos estas props para manejar la navegación y el filtrado
...
export function SubjectManager({ onSelectSubject, onNavigateToQuestions }: SubjectManagerProps) {
  ...
  const handleCardClick = (subjectId: string) => {
    onSelectSubject(subjectId); // Primero establecemos el filtro
    onNavigateToQuestions();    // Luego cambiamos la sección/vista
  };
      ...
      <div className="bg-blue-50 dark:bg-blue-900/20 border-l-4 border-blue-500 p-4 rounded-r-lg">
        <div className="flex items-center gap-3">
          ...
          <p className="text-sm text-blue-800 dark:text-blue-300 font-medium">
            Haga clic en cualquier tarjeta para filtrar el banco de preguntas.
          </p>
        </div>
      </div>
      ...
      {subjects.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {subjects.map((subject) => {
            ...
            const count = getQuestionCount(subject.id);

            return (
              <div
                key={subject.id}
                onClick={() => handleCardClick(subject.id)} // <--- Acción de Click
              ...
              >
                ...
                <div className="p-6">
                  ...
                  <p className="text-sm text-slate-500 mb-6">
                    Explorar contenido de {subject.name.toLowerCase()}
                  </p>
